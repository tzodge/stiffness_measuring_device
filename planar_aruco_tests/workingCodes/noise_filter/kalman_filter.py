...
def create_A_mat (dt):
    for i in range(6):
        A_empty[i,i+6] = dt
        # A_empty[i,i+12] = constant velocity state model, thetax, thetay thetaz are axis angles.

    return A_empty

def KF(mu_tm1, cov_tm1, z_t):
    z_t = z_t.reshape(6,1)
    random_mat = np.random.random((12,12))
    u,d,vt =  np.linalg.svd(random_mat )

    R_t = np.diag(np.random.random(12,))*R_t_coeff#
    # R_t=u.dot(np.diag(abs(d))).dot(vt)*R_t_coeff
    # R_t=np.diag(abs(d))*R_t_coeff
    ...
    cov_t = (np.eye(12) - K_t.dot(C_t)).dot(cov_t_bar)
...
A_empty = np.eye(12)   ###12x12
C_t = np.concatenate((np.eye(6),np.zeros((6,6))),axis=1)
Q = np.loadtxt("sens_noise_cov_mat.txt")

Q = Q*noise_coeff

state_cov_mat_tm1 = np.eye(12)
state_tm1 = np.zeros((12,1))
# pred_cov = 
...
while(j<iterations_for_while):
    ...
    if ids is not  None :
        ...
        if j > 0:
            ...
            print (time.time() - a), "processing time" 
            ...
            state_tm1 = state_tm1.reshape(12,1)
...
if detect_tip_switch == 1:
    ...
    ax.scatter(pose_marker[:,3],pose_marker[:,4],pose_marker[:,5],c = color[0:pose_marker.shape[0]])
    ...
    ax.set_ylabel('Y Label')
    ax.set_zlabel('Z Label')
    ax.scatter(pose_marker_filtered[:,3],pose_marker_filtered[:,4],pose_marker_filtered[:,5], c = color[0:pose_marker_filtered.shape[0]])
    ...
    ax.set_ylabel('Y Label')
    ax.set_zlabel('Z Label')
    ax.scatter(pose_marker_filtered[:,3],pose_marker_filtered[:,4],pose_marker_filtered[:,5])
    ax.scatter(pose_marker[:,3],pose_marker[:,4],pose_marker[:,5])
