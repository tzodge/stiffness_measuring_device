...
# 	std = [0]*valid_img_num
# 	dist_from_mean = [0]*valid_img_num
...
# 		sum1 = sum1 + predicted_loc[i]
# 	mu = sum1/valid_img_num	


# 	for i in range(valid_img_num):
# 		dist_from_mean[i] = predicted_loc[i]-mu
# 		sum2 = sum2 + (np.pi/2 - abs(rvecs[0,0][2])) * dist_from_mean[i]**2 
# 	# b = np.std(predicted_loc,axis=0)	

# 	# for i in range(valid_img_num):
			

# 	return sum2.sum()/valid_img_num
def obj_func(xyz_gs):
	sum2 = 0
	### x_gs,y_gs,z_gs are initial guesses
	gs_vect = np.array([xyz_gs[0],xyz_gs[1],xyz_gs[2],1])
	...
	for i in range(valid_img_num):
		for j in range(i+1,valid_img_num):
			sum2 = sum2 + np.linalg.norm((transf_mat_for_img[i]-transf_mat_for_img[j]).dot(gs_vect))
# 		sum1 = sum1 + predicted_loc[i]
	# b = np.std(predicted_loc,axis=0)	
	...
	return sum2/valid_img_num

print ""
### Switches: 
sub_pix_refinement_switch = 1
num_img_considered = 100
...
rodrigues_normals_vect = [0]*total_num_of_img 
valid_img_num = 0 
initial_guess = np.array([-3 ,-110.0,   -5.0])
...
for img in glob.glob('data_images/*.jpg'):
	...
	if ids is not None:
		...
		# print np.linalg.norm(rvecs)
		# print rvecs
		# print rvecs.shape
		# print np.pi/2 - abs(rvecs[0,0][2])
		# print '.....'
...
del(transf_mat_for_img[valid_img_num:])  #### DELETING ALL IN WHO COME AFTER THE VALID iMAGE NUMBER
