...
pose = pose_corr + np.array([0.00,0.00,0.00,np.random.rand(),np.random.rand(),20.])*1
print("pose",pose)
def error_DPR (pose,frame_gray):
    ...
    err = (y - f_p )/float(n_int) # this is the error in the intensities
    ii = 0
    for i in range(n_int):  
        ii+=1       
...
def DPR_GN(pose,frame_gray):
    ...
    while np.square(err).sum() <= err_tol and np.square(dp[0:3]).sum() <= ang_tol and np.square(dp[3:6]).sum() <= tra_tol or iter_ <= max_iter_GN:
        ...
        dI_by_dp, err = error_DPR (pose, frame_gray)
#       # solving with QR decomposition
        ...
        # while  err_new.sum() > err.sum() + AG_2*term_2_of_AG_cond and AG_iter_ < 20:
        #   print(AG_iter_,"AG_iter_")
        #   AG_iter_ += 1
        #   dp = alpha * dp
        #   pose = pose + dp 
        #   dI_by_dp, err_new = error_DPR (pose, frame_gray)
